...
export class AuthService {
  ...
  async createUserServices(createdUserDto: CreateUserDto) {
    ...
    const walletExisting = await this.usersRepository.getUserByWalletRepository(
      createdUserDto.wallet,
    );
    if (walletExisting) {
      throw new BadRequestException('Esta wallet ya está en uso');
    }
