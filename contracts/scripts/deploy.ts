...
async function main() {
  console.log("🚀 Desplegando contratos Gaia Protocol...");
  ...
  const token = await GaiaToken.deploy(initialSupply);
  await token.waitForDeployment();
  console.log("✅ GaiaToken desplegado en:", await token.getAddress());

  const GaiaNFT = await ethers.getContractFactory("GaiaNFT");
  const nft = await GaiaNFT.deploy(
    "Gaia Protocol NFT",
    "GNFT",
    deployer.address,
    deployer.address
  ...
  console.log("✅ GaiaNFT desplegado en:", await nft.getAddress());
