...
/// @title Gaia Protocol token (ERC20)
contract GaiaToken is ERC20, ERC20Burnable, Ownable {
    constructor(uint256 initialSupply)
        ERC20("Gaia Protocol", "GAIA")
        Ownable(msg.sender) 
    {
...
/// @title GaiaNFT (ERC721) 
contract GaiaNFT is ERC721URIStorage, Ownable {
    ...
    modifier onlyMinter() {
        require(minter == msg.sender, "GaiaNFT: caller is not minter");
        _;
    }
...
/// @title CampaignManager - gestiona campañas, fondos GAIA y minteo de NFTs
contract CampaignManager is Ownable, ReentrancyGuard {
        ...
        Ownable(msg.sender)
    {
        ...
        token = GaiaToken(tokenAddress);
        nft = GaiaNFT(nftAddress);
    }

    /// @notice Crea una nueva campaña
    function createCampaign(
        string calldata uuid_,
        ...
        require(bytes(uuid_).length > 0, "UUID required");
        ...
        emit CampaignCreated(id, uuid_, name_, creator_);
        return id;
    }
